...
const GrammarFeedbackPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 pb-32 p-6 bg-gray-50 min-h-screen">
      <div className="sticky top-0 z-10 bg-gray-50 flex items-center gap-2 py-4 mb-2">
        <Button 
          variant="ghost" 
          size="icon"
        ...
        <h1 className="text-2xl font-bold text-center flex-1 text-white">Grammar Feedback</h1>
      </div>

      <div className="flex flex-col gap-8">
        {grammarFeedbackData.map((item, index) => (
          <div key={index} className="flex flex-col gap-3 bg-white p-5 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="mb-2">
              <h2 className="text-lg font-bold text-primary">{item.title}</h2>
              <p className="text-base leading-relaxed mt-1 text-gray-800">{item.description}</p>
            </div>
            
            <div className="flex items-center gap-4 p-3 mt-1 border-2 border-blue-200 rounded-full bg-white shadow-sm">
              <Button
                variant="ghost"
                size="icon"
